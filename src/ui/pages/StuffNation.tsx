...
export default function StuffNation() {
  return (
    <div className="min-h-screen w-screen bg-white">
      <div className="bg-blue-100 text-gray-700 space-y-8 container mx-auto p-8">
        ...
        <div className="flex space-x-12">
          <div className="text-sm space-y-4 flex-shrink-0 cursor-pointer">
            <div>{'Platforms >'}</div>
            <div>{'Regions >'}</div>
            <div>{'Features >'}</div>
            <div>{'Rating >'}</div>
            ...
            <ul className="space-y-3">
              {products.map((product) => (
                <li className="flex w-full items-center space-x-3">
                  ...
                  <span className="flex-grow text-xl">{product.name}</span>
